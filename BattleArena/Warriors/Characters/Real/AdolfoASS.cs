using BattleArena.Combat;
using System;
using System.Threading;
using BattleArena.Enums;

namespace BattleArena.Warriors.Characters
{
    public class AdolfoASS : Warrior
    {
        public int CritMultiplier { get; private set; }

        public AdolfoASS(int health, int attackPower, int critMultiplier, int speed, TeamType teamType)
            : base("AdolfoASS", health, attackPower, speed, WarriorType.Assasin, teamType)
        {
            CritMultiplier = critMultiplier;
        }

        public override void Attack(Warrior target)
        {
            var dmginfo = new DamageInfo(AttackPower, "Tusok", HasCriticalChance, this);

            Console.WriteLine($"->{Name}: Strike {target.Name}...");
            Thread.Sleep(1000);

            if (dmginfo.IsCritical)
                Console.WriteLine($"-> Critical! Damage x{CritMultiplier}");

            TakeDamage(dmginfo);

            Thread.Sleep(1000);
            Console.WriteLine($"->* Damage Info: {dmginfo.TotalAmountDamage}");
            Console.WriteLine($"->* Target Health (approx): {target.Health}");
        }
    }
}
